...
var testDMLRepository = NewDML()

//var testDMLRepository = NewDMLWithOptions(&Options{TableName: "Tests"})

func TestDML_buildDeleteStmt(t *testing.T) {
	stmt := testDMLRepository.buildDeleteStmt(testRecord1)
	assert.Equal(t, "DELETE FROM `Tests` WHERE `String`=@w_String AND `Int64`=@w_Int64", stmt.SQL)
...
func TestDML_buildDeleteAllStmt(t *testing.T) {
	...
	assert.Equal(t, "DELETE FROM `Tests` WHERE (`String`=@w_String_0 AND `Int64`=@w_Int64_0) OR (`String`=@w_String_1 AND `Int64`=@w_Int64_1)", stmt.SQL)
